...
namespace NeverSayNever
{
    ...
    public class UIMgr : IUIMgr
    {
        ...
        {
            RegisterCsPanelByReflect(name);
        }

        public void Register4Lua(string name)
        {
            RegisterLuaPanel(name);
        }
        ...
        public void RegisterCsPanelByReflect(string moduleName)
        {
            ...
            var target = NsnRuntime.CreateInstance(messengerName, parameters);
            if (target == null)
            {
        ...
        public void ClosePanel(string moduleName)
        {
            _allPanelInfoDic.TryGetValue(moduleName, out var info);
            if (info == null)
            {
            ...
            if (panel != null)
            {
                mResMdl.ReleaseObject(panel.gameObject);
        ...
        private void LoadCsPanel(UIPanelMessenger messenger, params object[] args)
        {
            ...
            void LoadPanelComplete(object obj)
            {
                ...
                // 添加对应的脚本
                var panelScript = NsnRuntime.AddScript(panelObj, uiScript) as UIBasePanel;
                // 显示UI对象
            ...
            // 加载UI界面
            mResMdl.LoadUIAsset(panelName, LoadPanelComplete);
        }
        ...
        public void OnInitialize()
        {
            try
            {
                GameObject uiRoot = GameObject.Find("UICanvans");
                //var uiRoot = args[0] as GameObject;
                var uiPool = new GameObject("UIPool");
                Root = uiRoot.transform;
                PoolRoot = uiPool.transform;
